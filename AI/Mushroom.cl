...
)
{
    ...
    if(evol[elw + 4] > 1)
    {
        ...
        for(int i = 7; i < (int)size[3] / 3; i+=5)
            red += evol[elw + i];
        ...
        float sf1 = evol[f1 + 4] > 0 ? 1 : 0;
        float sf2 = evol[f2 + 4] > 0 ? 1 : 0;
        float sf3 = evol[f3 + 4] > 0 ? 1 : 0;
        float sf4 = evol[f4 + 4] > 0 ? 1 : 0;

        for(int i = 7; i < size[3]; i+=5)
        {
        ...
        Mul(vec2, vec1, evol, 7, 30, id, 7 + elw);
        CopyVec(vec1, vec2, vecw, size[1]);
        Mul(vec2, vec1, evol, 30, 7, id, 217 + 0 + elw);
        ...
        if(action == 1)                             //ATTACK
        {
            ...
            evol[elw + 5] = evol[elw + 5] + evol[dir + 5]/2 - 0;
            evol[elw + 5] = 0;
